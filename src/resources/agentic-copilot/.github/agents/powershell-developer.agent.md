...
- When help files change, keep `docs/{{ProjectName}}/en-US/*.md` valid for `Import-MarkdownCommandHelp` and follow this workflow:
	1. Build and import the dist module: `Import-Module ./dist/{{ProjectName}}/{{ProjectName}}.psd1 -Force`.
	...
	4. If you encounter an existing help file where the `external help file` field contains a command name rather than `{{ProjectName}}-Help.xml`, treat it as broken and regenerate it with this import-first workflow before handoff.
