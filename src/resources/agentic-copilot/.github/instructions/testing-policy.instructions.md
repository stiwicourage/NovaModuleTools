...
- Cross-cutting `*.Integration.Tests.ps1` files may `Import-Module ./dist/<ModuleName>/<ModuleName>.psd1` when the purpose is validating built-module behavior, exported command shape, or build-validation smoke coverage.
...
- Use `Invoke-NovaTest` for unit-test validation and `Test-NovaBuild` for build-validation integration coverage in Nova-managed projects. Do not validate with direct `Invoke-Pester`, because it can bypass Nova's build/import/StrictMode flow and disagree with what users see later.
