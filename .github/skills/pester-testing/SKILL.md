...
- If you encounter an existing test file that uses `Import-Module $project.OutputModuleDir` or `InModuleScope NovaModuleTools`, migrate it to the dot-source pattern as part of the same change. If migration is out of scope for the current task, add a TODO comment in that test file and note it in your final handoff.
