...
This skill applies even when the surface is not named "CLI". In NovaModuleTools, PowerShell cmdlets are also terminal UX.
...
5. **Craft human-readable error messages**  
   ...
   PowerShell cmdlets: surface actionable `Stop-NovaOperation` messages and preserve a stable `ErrorId`.  
...
| Recoverable errors      | `Stop-NovaOperation` with stable `ErrorId`, plain language, and actionable recovery guidance                                                            | 5                      | 9                    |
