...
- Treat available CodeScene maintainability and changed-code coverage results as release-blocking signals unless risk is accepted explicitly. If local CodeScene tooling is unavailable, note that local CodeScene was not run and rely on PR/CI as the effective CodeScene gate instead of treating the missing local result as a blocker by itself.
...
- Check that public commands/classes have matching valid PlatyPS-compatible help. Flag help files under `docs/NovaModuleTools/en-US/` that look like plain Markdown, break the required PlatyPS section order, or would fail `Test-MarkdownCommandHelp` / `Import-MarkdownCommandHelp`.
