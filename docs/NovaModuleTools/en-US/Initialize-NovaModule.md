...
`run.ps1` must be fixed before handoff, explicit public/private PowerShell file ownership rules, a best-effort src/tests quality matrix delivered through Agentic Copilot guidance files instead of a required `.codescene/code-health-rules.json`, and a strict file-ending rule for changed or generated text files. For the standard scaffold, the interactive flow also includes optional basic Pester support.
