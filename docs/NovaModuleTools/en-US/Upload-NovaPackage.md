...
```powershell
PS> Upload-NovaPackage [[-PackagePath] <string[]>] [[-PackageType] <string[]>] [[-Url] <string>] [[-Repository] <string>] [[-UploadPath] <string>] [[-Headers] <hashtable>] [[-Token] <string>] [[-TokenEnvironmentVariable] <string>] [[-AuthenticationScheme] <string>] [-WhatIf] [-Confirm] [<CommonParameters>]
```

## DESCRIPTION

`Upload-NovaPackage` uploads existing package artifacts that were generated by `Pack-NovaModule`.
...
When multiple matching artifacts exist for a selected package type, `Upload-NovaPackage` uploads all of them. That
...
```powershell
PS> Upload-NovaPackage -Url 'https://packages.example/raw/' -Token $env:NOVA_PACKAGE_TOKEN
```
...
```powershell
PS> Upload-NovaPackage -PackageType Zip -Repository LocalNexus
```
...
```powershell
PS> Upload-NovaPackage -PackagePath @('artifacts/packages/NovaModuleTools.2.0.0-preview6.zip', 'artifacts/packages/NovaModuleTools.latest.zip') -Url 'https://packages.example/raw/releases/'
```
...
```powershell
PS> Upload-NovaPackage -Repository LocalNexus -WhatIf
```
...
Use `Pack-NovaModule` first when you want NovaModuleTools to generate the package artifacts before upload.

If no upload target can be resolved, `Upload-NovaPackage` fails fast with a clear error that tells you to provide
`-Url` or configure upload settings under the top-level `Package` section in `project.json`.

## RELATED LINKS

- https://github.com/stiwicourage/NovaModuleTools/blob/main/docs/NovaModuleTools/en-US/Pack-NovaModule.md
